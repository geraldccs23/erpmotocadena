      ...
      <div className="space-y-6 md:space-y-8 animate-in slide-in-from-right-10 duration-500 pb-20">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex gap-2 items-center">
             {isBilled && (
               <span className={`px-3 py-1 rounded-lg text-[8px] md:text-[10px] font-black uppercase tracking-[0.2em] border ${
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
          <div className="lg:col-span-2 space-y-6 md:space-y-8">
            <div className="glass-panel p-6 md:p-10 rounded-2xl md:rounded-[3rem] border border-white/5 relative overflow-hidden shadow-2xl">
              <div className="flex flex-col sm:flex-row gap-6 items-start relative z-10">
                <div className="w-16 h-16 md:w-24 md:h-24 bg-zinc-950 border border-zinc-800 rounded-2xl md:rounded-[2rem] flex items-center justify-center text-amber-500 shrink-0">
                  <Bike size={32} className="md:w-12 md:h-12 drop-shadow-[0_0_10px_rgba(245,158,11,0.5)]" />
                </div>
                <div className="space-y-2 md:space-y-4">
                  <div>
                    <h2 className="heading-racing text-4xl md:text-7xl text-zinc-100 italic leading-none tracking-tighter uppercase">Orden #{currentOrder.id.slice(0, 8).toUpperCase()}</h2>
                    <p className="text-amber-500 font-bold heading-racing text-2xl md:text-4xl tracking-widest mt-1 italic">
                      {currentOrder.vehicle?.plate} • {currentOrder.vehicle?.brand} {currentOrder.vehicle?.model}
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-2 text-[8px] md:text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500">
                    <span className="bg-zinc-900 px-3 py-1.5 rounded-lg border border-zinc-800 flex items-center gap-2"><User size={12}/> {currentOrder.customer?.first_name}</span>
                    <span className="bg-zinc-900 px-3 py-1.5 rounded-lg border border-zinc-800 flex items-center gap-2"><Gauge size={12}/> {currentOrder.mileage} KM</span>
                  </div>
                </div>
              </div>
            </div>

            <div className="glass-panel p-6 md:p-10 rounded-2xl md:rounded-[3rem] border border-white/5 space-y-6 md:space-y-10">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <h3 className="heading-racing text-3xl md:text-5xl text-zinc-100 italic uppercase">Liquidación</h3>
                <div className="flex gap-2">
                  <button onClick={() => setShowItemSearch('SERVICE')} className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-zinc-950 border border-zinc-800 rounded-lg text-[8px] font-black text-amber-500 uppercase"><Layers size={14}/> +Servicio</button>
                  <button onClick={() => setShowItemSearch('PART')} className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-zinc-950 border border-zinc-800 rounded-lg text-[8px] font-black text-blue-500 uppercase"><Package size={14}/> +Repuesto</button>
                </div>
              </div>
              ...
              <div className="pt-6 border-t border-zinc-900 space-y-2">
                 <div className="flex justify-between text-zinc-600 text-[8px] md:text-[10px] font-black uppercase tracking-widest">
                    <span>Subtotal</span>
                    <span className="text-zinc-100">${Number(currentOrder.total_amount || 0).toFixed(2)}</span>
                 </div>
                 <div className="flex justify-between pt-4 bg-zinc-950/30 p-4 rounded-xl border border-zinc-900">
                    <span className="heading-racing text-2xl md:text-4xl text-zinc-100 italic">TOTAL</span>
                    <span className="heading-racing text-4xl md:text-6xl text-amber-500 text-glow-amber italic leading-none">${Number(currentOrder.total_amount || 0).toFixed(2)}</span>
                 </div>
              </div>
            </div>
          </div>
            ...
            <div className="glass-panel p-6 md:p-8 rounded-2xl md:rounded-[3rem] border border-white/5">
               ...
               <div className="space-y-4">
                  ...
                  <div className="space-y-2 pt-2">
                    <label className="text-[8px] md:text-[10px] font-black text-zinc-600 uppercase tracking-widest">Estatus</label>
                    <div className="grid grid-cols-2 gap-1.5">
                       {['DRAFT', 'APPROVED', 'IN_PROGRESS', 'WAITING_PARTS', 'READY'].map(s => (
                         <button 
                          key={s} 
                          onClick={() => updateOrderField(currentOrder.id, 'status', s)}
                          className={`py-2 px-2 rounded-lg text-[7px] font-black uppercase tracking-widest border transition-all ${currentOrder.status === s ? 'bg-amber-500 border-amber-500 text-black' : 'bg-zinc-900 border-zinc-800 text-zinc-600'}`}
                         >
                           {s.replace('_', ' ')}
                         </button>
                       ))}
                  ...
                  <div className="max-h-48 overflow-y-auto scrollbar-hide space-y-1.5">
                    {INSPECTION_ITEMS.map(item => (
                      <button 
                        ...
                        className={`w-full flex items-center justify-between p-3 rounded-lg border transition-all ${currentOrder.initial_inspection?.[item.id] ? 'bg-emerald-500/5 border-emerald-500/30 text-emerald-500' : 'bg-zinc-950 border-zinc-900 text-zinc-600'}`}
                      >
                        <span className="text-[8px] md:text-[10px] font-bold uppercase tracking-widest truncate">{item.label}</span>
                        <CheckCircle2 size={14} className={currentOrder.initial_inspection?.[item.id] ? 'opacity-100' : 'opacity-10'} />
                      </button>
                    ))}
                  </div>
               </div>
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                 ...
                 <div className="space-y-1">
                   ...
                   <select required disabled={!selectedCustomer} value={selectedVehicle?.id || ''} onChange={e => {
                     const v = selectedCustomer?.vehicles?.find(v => v.id === e.target.value);
